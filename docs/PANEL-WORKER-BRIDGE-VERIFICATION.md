...
- `smoke-panel-workers` must pass and includes assertions that:
  ...
  - worker-produced task, result, log, launcher, mailbox, and lifecycle files are written inside each worker worktree;
  - tracked leader workspace changes are visible in worker worktrees before launch;
  - `panel resume` ignores stale worker result artifacts from prior attempts;
  - native worker tasks forbid hidden reasoning persistence;
  - raw worker result files are re-serialized to strict final-output fields before aggregation;
  - handoff includes native worker guidance and worker evidence references;
  - `longtable team` exits with `Unknown command: team`, writes no stdout, and creates no `.longtable/team` artifact in a fresh probe workspace.
...
- Fresh-workspace artifact probe: `.longtable/team` — PASS, not created by `longtable team`
