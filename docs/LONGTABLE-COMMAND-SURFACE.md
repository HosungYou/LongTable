...
`lt explore: ...`, `lt review: ...`, `lt methods: ...` 같은 짧은 형식은 별도 agent 파일이 아니라 LongTable directive parser와 role router에 기반합니다. Codex prompt alias나 Claude skill이 설치되어 있으면 같은 자연어 표면을 provider-native entrypoint로도 노출합니다.
...
이 명령은 `/prompts:longtable`, `/prompts:longtable-panel`, `/prompts:longtable-methods` 같은 prompt alias를 생성합니다.
