...
- Researcher Checkpoint -> adapter presentation concern through native `AskUserQuestion` when available
...
Claude Code의 native question tool은 버리지 않는다. LongTable core는 Researcher Checkpoint activation과 decision semantics를 소유하고, Claude adapter는 그 결과를 native structured question input으로 변환한다.
