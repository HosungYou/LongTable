  ...
  const base = [
    {
      ...
      body: [
        ...
        "- `$longtable-interview` to run the unified interview surface: structured follow-up after a Research Specification exists, or critical/grill-me mode when the researcher asks for pressure testing.",
        "- `$critical-interview` as a compatibility alias for `$longtable-interview` critical mode.",
        ...
        "- `$longtable-interview` is the unified interview surface. Ordinary option-first follow-up requires a usable Research Specification; critical/grill-me requests stay in `$longtable-interview` critical mode.",
        ...
        "- For `$longtable-interview`, use option-first follow-up choices only after a usable Research Specification exists. For critical/grill-me mode, ask one relentless sharpening question at a time, and stop when remaining questions repeat the same tension without producing a new decision. Always include an escape hatch such as Other, free text, or one open follow-up question when presenting options.",
    ...
    },
    {
      ...
      description:
        "Compatibility alias for `$longtable-interview` critical mode.",
      disableModelInvocation: true,
      body: [
        "Compatibility alias: run `$longtable-interview` in Critical Interview Mode. Do not run a separate interview contract here."
      ]
    },
    {
    ...
    },
    {
      ...
      description:
        "Use for `$longtable-interview`: run the unified LongTable interview surface, including option-first follow-up after a usable Research Specification and critical/grill-me pressure testing when requested.",
      body: [
        "## Purpose",
        "",
        "Run the unified LongTable interview inside Codex. It handles structured follow-up decisions after a usable Research Specification exists and critical pressure-testing when the researcher asks to be grilled.",
        "",
        "## Required Routing",
        "",
        ...
        "- If the request asks for critical interview, pressure testing, grilling, or relentless challenge, stay in this skill and run Critical Interview Mode.",
        "- For ordinary follow-up, if no usable Research Specification exists, route to `$longtable-start` immediately.",
        "- For ordinary follow-up, if only a First Research Shape exists, route to `$longtable-start` and continue into the next Research Specification question or preview.",
        ...
        "- The user invokes `$critical-interview`; treat it as a compatibility alias for this skill's Critical Interview Mode.",
        ...
        "",
        "## Critical Interview Mode",
        "",
        "- Run a relentless LongTable interview to sharpen a research plan, argument, method, manuscript, or product decision.",
        "- Ask one question at a time. Each question must expose a decision, hidden assumption, weak boundary, unsupported claim, or evidence gap.",
        ...
        "- If `.longtable/` exists, record the result as a `DecisionRecord`, Research Specification patch, or explicit open tension when MCP/state tools are available.",
        "",
        "## Core Flow",
        "",
        "1. Read the current Research Specification when available and identify whether the request is ordinary follow-up or Critical Interview Mode.",
        "2. For ordinary follow-up, present a small option-first choice set tied to the current specification.",
        "3. Include an escape hatch such as `Other`, free text, or `ask one open question first`.",
        "4. For Critical Interview Mode, ask the next highest-leverage sharpening question instead of a menu.",
        ...
        "6. Record the resulting decision as a `DecisionRecord` or explicit open tension; never silently overwrite conflicting research commitments.",
        "",
        "## Option UI Policy",
        "",
